...
Crawler.prototype.init = function init (options) {
    ...
    self.pool = genericPool.createPool(poolFactory, {
...
Crawler.prototype.shutdown = function(){
    var that = this;
    return that.pool.drain() 
    .then(function(){
        return that.pool.clear(); 
    });
}
...
Crawler.prototype._release = function _release(options){
    ...
    if (self.pool.isBorrowedResource(options._poolReference)){
        p = p.then(function(){
            return self.pool.release(options._poolReference);
        });
    }
...
Crawler.prototype._pushToQueue = function _pushToQueue (options) {
    ...
    return self.pool.acquire(options.priority)
    .then(function(poolReference){
...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest (options) {
    ...
    // options objects that need to be preserved
    _.each(['jar'], function(k, i){
        if (k in options) ropts[k] = options[k]; 
    });
    ...
    // request doesn't handle deflate encoding
    if (ropts.deflate){
        ropts.encoding = null;
    }
    ...
    if (ropts.cloudflare){
        requestlib = cloudscraper.request;
        if (! ropts.url) ropts.url = ropts.uri;
    }

    self.emit('request.sent', ropts);

    var req = requestlib(ropts, function(error,response, body) {
        self.emit('response.received', ropts);

        if (error) {
            return self._onContent(error, options);
        }
        response = response || {};
        if (! response.body && body) response.body = body;

        response.uri = response.request.href;

        // handle deflate, request does not currently support it
        if (ropts.deflate &&
            response.headers &&
            typeof(response.headers['content-encoding']) === 'string' &&
            response.headers['content-encoding'].trim().toLowerCase() === 'deflate'){
            zlib.inflateRaw(response.body, function (error, body) { 
                if (error) {
                    return self._onContent(error, options);
                }
...
Crawler.prototype._onContent = function _onContent (error, options, response, fromCache) {
    ...
    var isHTML = typeof(response.body) === 'string' ? response.body.match(/^\s*</) : false;
